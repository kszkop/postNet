contentAnalysis <- function(annot,
  ...
  for (i in 1:length(contentIn)) {
    ...
    if (isTRUE(plotOut)) {
      ...
      # Plot
      pdf(ifelse(is.null(pdfName), paste(region, content, "contentAnalysis.pdf", sep = "_"), paste(pdfName, region, content, "contentAnalysis.pdf", sep = "_")), width = 8, height = 8, useDingbats = F)
    ...
    contentFinal[[paste(region, content, sep = "_")]] <- contentOut
  }
  #
  return(contentFinal)
}
