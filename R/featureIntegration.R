featureIntegration <- function(features,
  ...
  if (is.null(geneList)) {
    #
    if (isTRUE(regOnly)) {
      resTmp <- anota2seqGetDirectedRegulations(ads)[[contrastSel]]
      #
      listSel <- as.character(unlist(resTmp[grepl(regulationGen, names(resTmp))]))
      #
  ...
  if (analysis_type == "rf") {
    if (isTRUE(regOnly)) {
      ...
      if (is.null(geneList)) {
        dat$reg <- 0
        dat$reg[row.names(dat) %in% as.character(resTmp[grepl(regulationGen, names(resTmp))][[1]])] <- "A"
        dat$reg[row.names(dat) %in% as.character(resTmp[grepl(regulationGen, names(resTmp))][[2]])] <- "B"
      } else {
        for (i in 1:length(geneList)){
          dat$reg <- 0
          dat$reg[row.names(dat) %in% as.character(unlist(resTmp[c(i)]))] <- i
        }
      }
    }
    ...
    #
    pdf(ifelse(is.null(pdfName), paste(ifelse(is.null(regulationGen), "custom", regulationGen), "featureSel_randomForest.pdf", sep = "_"), paste(pdfName, ifelse(is.null(regulationGen), "custom", regulationGen), "featureSel_randomForest.pdf", sep = "_")), width = 8, height = 8, useDingbats = F)
    ...
    coloursN[tmpNames %in% addNames] <- "firebrick1"
      ...
      #
      pdf(ifelse(is.null(pdfName), paste(ifelse(is.null(regulationGen), "custom", regulationGen), "randomForest.pdf", sep = "_"), paste(pdfName, ifelse(is.null(regulationGen), "custom", regulationGen), "randomForest.pdf", sep = "_")), width = 16, height = 8, useDingbats = F)
  ...
  for (feat in bestSel) {
    ...
    pdf(ifelse(is.null(pdfName), paste(ifelse(is.null(regulationGen), "custom", regulationGen), featTmp, "individually.pdf", sep = "_"), paste(pdfName, ifelse(is.null(regulationGen), "custom", regulationGen), featTmp, "individually.pdf", sep = "_")), width = 8, height = 8, useDingbats = F)
  ...
  pdf(ifelse(is.null(pdfName), paste(ifelse(is.null(regulationGen), "custom", regulationGen), featTmp, "heatmap.pdf", sep = "_"), paste(pdfName, ifelse(is.null(regulationGen), "custom", regulationGen), featTmp, "heatmap.pdf", sep = "_")), useDingbats = F, width = 20, height = 24)
