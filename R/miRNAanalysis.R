miRNAanalysis <- function(ptn,
  ...
  if (Pct == 0 && contextScore == 0) {
    message("Please consider providing filtering thresholds for 'contextScore' or 'Pct'.")
  }
  if (Pct < 0 || Pct > 1) {
  ...
  if (length(miRNAList) == 0){
    stop("No miRNA meet the selected thresholds. Please adjust 'contextScore' and/or 'Pct' for less stringent flitering.")
  }
