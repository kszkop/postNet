codonUsage <- function(ptn,
  ...
  if (!is_annotType(annotType)) {
    stop(
      "Please provide an input for 'annotType' specifying the source of annotation. The options are 'ccds', or 'ptnCDS'."
    )
  }
  if (annotType == "ccds") {
    ...
    if (sourceSeq == "create") {
      ...
      if (!species %in% currTmp) {
        stop(
          "This option is currently only available for human and mouse. For analyses on other species or \
  ...
  if (analysis == "codon" & codonN == 1) {
    ...
    if (species == "human") {
      codind <- read.delim(
        system.file("extdata/indexes/human/", "IndexesHuman.txt", package = "postNet")
      )
    } else if (species == "mouse") {
      codind <- read.delim(
        system.file("extdata/indexes/mouse/", "IndexesMouse.txt", package = "postNet")
      )
    } else {
  ...
  for (j in seq_along(comparisons)) {
    ...
    if (all(resIn <= 0)) {
      stop(
        "The contingency table (codon or AA counts by geneset) must have at least one positive entry \
    ...
    if (sum(apply(resIn, 2, min) < 5) > 0) {
      if (isTRUE(rem5)) {
        ...
        warning(
          "In the contingency table (codon or AA counts by geneset), some counts are lower than 5 which may invalidate the Chi-squared test. \ You may wish to perform the analysis on a subset or groups of codons or AAs instead.",
          call. = FALSE
        )
      }
    }
    ...
    if (length(signSel) == 0) {
      ...
      if (analysis == "codon" & codonN == 1) {
        ...
        if (length(codU) > 0) {
          ...
          message(
            "None of the codons are labelled with the specified thresholds. \
        ...
        if (length(codU) > 0) {
          ...
          message(
            "None of the AAs are labelled with the specified thresholds. \
                  Please select more relaxed thresholds for thresOddsUp and thresFreqUp."
          )
        }
        #
