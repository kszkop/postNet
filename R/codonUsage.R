codonUsage <- function(
    ...
    if (!is_annotType(annotType)) {
        stop(
            "Please provide an input for 'annotType' specifying the ",
            "source of annotation. The options are 'ccds', or 'ptnCDS'."
        )
    }
    ...
    if (annotType == "ccds") {
        ...
        if (!is_valid_sourceSeq(sourceSeq)) {
            stop(
                "Please provide an input for 'sourceSeq'. The options ",
                "are: 'load' or 'create'."
            )
        }
        ...
        if (sourceSeq == "create") {
            if (species == "human") {
                ...
                ccdsSeq <- seqinr::read.fasta(
                    "CCDS_nucleotide_human.fna",
                    seqtype = "AA"
                )
                ...
                ccdsSeq <- ccdsSeq[
                    !duplicated(sub("\\|.*", "", names(ccdsSeq)))
                ]
                
                ccdsSeq <- data.frame(
                    id = sub("\\..*", "", names(ccdsSeq)),
                    CDS_seq = t(as.data.frame(lapply(
                        ccdsSeq,
                        function(x) {
                            paste(x, collapse = "")
                        }
            ...
            if (!species %in% currTmp) {
                stop(
                    "This option is currently only available for human ",
                    "and mouse. For analyses on other species or ",
                    "annotations, please use the source = 'custom' ",
                    "option in the postNetStart() function to provide ",
    ...
    if (!is_valid_analysis(analysis)) {
        stop(
            "Please provide an input for 'analysis'. The options are ",
            "'codon' or 'AA' (amino acid)."
        )
    }
    
    if (!check_number(codonN)) {
        stop(
            "Please provide an integer value for 'codonN' specifying ",
            "the number of consecutive codons to consider in the ",
            "analysis. For example, 1 will consider each individual ",
            "codon, while 2 will consider dicodons, etc."
        )
    }
    ...
    if (!is.null(comparisons)) {
        if (!check_comparisons(comparisons)) {
            stop(
                "The input for 'comparisons' must be a list of numeric ",
                "vectors of paired comparisons. For example: ",
                "list(c(0, 2), c(0, 1)). 0 always denotes the ",
                "background gene set."
            )
        }
        
        if (
            length(which(unique(unlist(comparisons)) == 0)) > 0 &&
            is.null(ptn_background(ptn))
        ) {
            stop(
                "0 always denotes the background, but no background ",
                "has been provided."
            )
        }
    } else {
    ...
    if (analysis == "codon" && codonN == 1) {
        ...
        if (!is_valid_species(species)) {
            stop(
                "Currently, codon usage indexes are only available for ",
                "'human' or 'mouse'."
            )
        }
        
        if (species == "human") {
            codind <- read.delim(system.file(
                "extdata/indexes/human/",
                "IndexesHuman.txt",
                package = "postNet"
            ))
        } else if (species == "mouse") {
            codind <- read.delim(system.file(
                "extdata/indexes/mouse/",
                "IndexesMouse.txt",
                package = "postNet"
            ))
        } else {
    ...
    for (j in seq_along(comparisons)) {
        ...
        if (all(resIn <= 0)) {
            stop(
                "The contingency table (codon or AA counts by geneset) ",
                "must have at least one positive entry for the ",
                "Chi-squared test to be performed."
            )
        }
        ...
        if (sum(apply(resIn, 2, min) < 5) > 0) {
            if (isTRUE(rem5)) {
                ...
                warning(
                    "In the contingency table (codon or AA counts by ",
                    "geneset), some counts are lower than 5 which may ",
                    "invalidate the Chi-squared test. You may wish to ",
                    "perform the analysis on a subset or groups of ",
                    "codons or AAs instead.",
                    call. = FALSE
                )
            }
        }
        ...
        if (is.na(chisqTest$p.value)) {
            stop(
                "The Chi-squared test could not be performed. Please ",
                "check that all codons or AAs have at least one count ",
                "in at least one gene set."
            )
        }
        
        StResiduals <- chisqTest$stdres
        signifLimit <- sqrt(qchisq(
            pAdj / (dim(resIn)[1] * dim(resIn)[2]),
            lower.tail = FALSE,
            df = 1
        ))
        ...
        if (isTRUE(plotHeatmap) && codonN == 1) {
            ...
            pdf(
                paste(
                    nameOut,
                    paste(colnames(residOut), collapse = "_"),
                    "heatmap.pdf",
                    sep = "_"
                ),
                width = 20,
        ...
        signSel <- colnames(StResiduals)[
            which(abs(StResiduals[1, ]) > signifLimit)
        ]
        ...
        if (length(signSel) == 0) {
            ...
            if (analysis == "codon" && codonN == 1) {
                ...
                resIn2$AA <- seqinr::aaa(seqinr::translate(seqinr::s2c(
                    seqinr::c2s(rownames(resIn2))
                )))
                ...
                pOut1 <- ggplot2::ggplot(
                    resIn2,
                    ggplot2::aes(
                        !!sym(colnames(resIn2)[1]),
                        !!sym(colnames(resIn2)[2]),
                        col = AA
                    )
                ) +
                    ggplot2::theme_bw() +
                ...
                resIn3$AA <- seqinr::aaa(seqinr::translate(seqinr::s2c(
                    seqinr::c2s(rownames(resIn3))
                )))
                ...
                pOut2 <- ggplot2::ggplot(
                    resIn3,
                    ggplot2::aes(
                        !!sym(colnames(resIn3)[1]),
                        !!sym(colnames(resIn3)[2]),
                        col = AA
                    )
                ) +
                    ggplot2::theme_bw() +
                    ...
                    ggplot2::labs(
                        x = paste(
                            colnames(resIn3)[1],
                            "\n(Amino acid normalised codon usage)",
                            sep = ""
                        ),
                        y = paste(
                            colnames(resIn3)[2],
                ...
                resIn4$AA <- seqinr::aaa(seqinr::translate(seqinr::s2c(
                    seqinr::c2s(resIn4$codon)
                )))
                ...
                pdf(
                    paste(
                        nameOut,
                        paste(regComb[, 1], collapse = "_"),
                        "codon_oddratio_vs_freq.pdf",
                        sep = ""
                    ),
                    width = 8,
                    height = 8,
                ...
                legend(
                    -xlimT,
                    0.5,
                    paste(regComb[1], "vs", regComb[2], sep = " "),
                    bty = "n",
                    horiz = TRUE,
                    xpd = TRUE,
                    y.intersp = 2.8,
                ...
                codU <- names(sumFreq_up)[
                    sumFreq_up >= quantile(sumFreq_up, 1 - thresFreqUp) &
                ...
                if (length(codU) > 0) {
                    ...
                    message(
                        "None of the codons are labelled with the specified ",
                        "thresholds. Please select more relaxed thresholds ",
                        "for thresOddsUp and thresFreqUp."
                    )
                }
                ...
                codD <- names(sumFreq_down)[
                    sumFreq_down >= quantile(sumFreq_down, 1 - thresFreqDown) &
                ...
                if (length(codD) > 0) {
                    ...
                    message(
                        "None of the codons are labelled with the specified ",
                        "thresholds. Please select more relaxed thresholds ",
                        "for thresOddsDown and thresFreqDown."
                    )
                }
                
                dev.off()
                
                codonsSel[[paste(regComb, collapse = "_vs_")]] <- list(
                    Up = codU,
                    Down = codD
                )
            } else if (analysis == "AA") {
                ...
                if (length(codU) > 0) {
                    ...
                    message(
                        "None of the AAs are labelled with the specified ",
                        "thresholds. Please select more relaxed thresholds ",
                        "for thresOddsUp and thresFreqUp."
                    )
                }
                ...
                codonsSel[[paste(row.names(resIn), collapse = "_vs_")]] <-
                    signSel
            }
        }
    }
