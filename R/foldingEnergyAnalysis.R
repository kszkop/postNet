foldingEnergyAnalysis <- function(ptn,
  ...
  if (!check_logical(residFE)) {
    stop(
      "The input for 'residFE' (specifying whether the values should be normalised for the sequence length) must be logical: TRUE or FALSE."
    )
  }
  #
  if (sourceFE == "custom") {
    if (is.null(customFileFE)) {
      stop(
        "Please provide a custom file with folding energies using the 'customFileFE' parameter."
      )
    }
  }
  if (sourceFE == "load") {
    if (!is_valid_species(species)) {
      stop(
        "The species stored in the 'postNetData' object is not supported. Currently 'human' or 'mouse' are available."
      )
    }
  }
  if (sourceFE == "custom") {
      ...
      if (isTRUE(plotOut)) {
        ...
        pdf(
          ifelse(
            ...
            paste(pdfName, reg, plotType, "foldEnergyAnalysis.pdf", sep = "_")
          ),
          width = 8,
          height = 8,
          useDingbats = FALSE
    ...
    stop(
      "The selection of the folding energy source file is not valid. Please check the inputs and refer to the help manual for details."
    )
  }
}

runFE <- function(energyIn, residFE = FALSE, ptn) {
  #
