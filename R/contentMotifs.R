contentMotifs <- function(ptn,
    ...
    if (!is.null(comparisons)) {
        if (!check_comparisons(comparisons)) {
            stop(
                "The input for 'comparisons' must be a list of numeric vectors of paired comparisons. For example: list(c(0,2),c(0,1)). 0 always \
    ...
    if (!check_number(dist)) {
        stop(
            "For 'dist', please provide a numeric value specifying the minimal distance between motifs."
        )
    }
    ...
    if (!check_logical(resid)) {
        stop(
            "The input for 'resid' (specifying whether the values should be normalised for the sequence length) must be logical: TRUE or FALSE."
        )
    }
    ...
    if (!is_motifs(motifsIn)) {
        stop(
            "The input for 'motifsIn' should be a character vector of sequence motifs to be detected and quantified. Ambiguities can be \
        ...
        if (tolower(seqType) == "protein") {
            if (!is_by_3(seqTmp)) {
                stop(
                    ...
                    into codons (are multiples of 3) 
                    so cannot be translated into protein sequences."
                )
            }
        ...
        if (!is.null(subregion)) {
            ...
            if (length(which(is.na(subSeq))) > 0) {
                message(
                    ...
                    than the sequence region. These sequences will be removed 
                    from the analysis."
                )
            }
            seqTmp <- subSeq
        ...
        for (i in seq_along(motifsIn)) {
            ...
            if (motif == "G4" & !tolower(seqType) == "protein") {
                if (!check_number(min_score)) {
                    stop(
                        "Please provide a numeric value specifying
                        the minimal score 
                        for G-quadruplexes selection ('min_score' parameter)."
                    )
                }
                motifOutTmp <- vapply(seqTmp,
                                                            calc_g4,
                                                            numeric(1),
                                                            min_score = min_score,
                                                            unit = unitOut)
            ...
            if (tolower(unitOut) == "number") {
                if (length(which(motifOutTmp > 0))) {
                    if (isTRUE(resid)) {
                        lenTmp <- vapply(seqTmp, function(x) {
                            length(seqinr::s2c(x))
                        }, integer(1))
                    ...
                    if (tolower(unitOut) == "number" & isTRUE(plotOut)) {
                        nameTmp <- ifelse(
                            ...
                            paste(pdfName, reg, motif, "content.pdf", sep = "_")
                        )
                        nameOut <- nameTmp
                ...
                if (!any(!vapply(motifOutTmp, function(x) {
                    all(is.na(x))
                }, logical(1)))) {
                    message(motifTmp, " does not have any sites")
                } else {
