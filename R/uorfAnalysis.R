uorfAnalysis <- function(
    ...
    if (!isStartCodon(startCodon)) {
        stop(
            "The input for 'startCodon' must be a character vector of ",
            "length one, and contain a 3-nucleotide sequence, ex. 'ATG'."
        )
    }
    
    if (!isKozakContext(KozakContext)) {
        stop(
            "The input for 'KozakContext' must be either: 'strong', ",
            "'adequate1', 'adequate2', 'weak', or 'any'."
        )
    }
    ...
    if (tolower(unitOut) == "number" && isTRUE(plotOut)) {
        ...
        if (length(resOut) == 0) {
            stop(
                "There are no regulated genes. Check the input or run ",
                "without indicating 'regulation' and 'comparisons'."
            )
        }
        ...
        for (i in seq_along(resOut)) {
            resProp[i] <- length(resOut[[i]][resOut[[i]] > 0]) /
                length(resOut[[i]])
        }
        ...
        for (i in xpos) {
            rect(
                xleft = xpos[i] - bar_width / 2,
                xright = xpos[i] + bar_width / 2,
                ybottom = 0,
                ytop = resProp[i],
                col = colOut[i],
                border = NA
