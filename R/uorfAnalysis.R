uorfAnalysis <- function(ptn,
  ...
  if (!isStartCodon(startCodon)) {
    stop(
      "The input for 'startCodon' must be a character vector of length one, and contain a 3-nucleotide sequence, ex. 'ATG'"
    )
  }
  if (!isKozakContext(KozakContext)) {
    stop(
      "The input for 'KozakContext' must be either: 'strong', 'adequate1', 'adequate2', 'weak', or 'any'."
    )
  }
  ...
  if (tolower(unitOut) == "number" & isTRUE(plotOut)) {
    ...
    axis(
      ...
      las = 2,
      lwd = 2
    ) # ), at = seq(0, ylimTmp[2], 0.2), labels = seq(0, 1, 0.2))
    text(
      x = xpos,
