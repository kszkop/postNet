postNetStart <- function(ads = NULL,
  ...
  if(length(setdiff(customBg, outDB$geneID))>0){
    warning(paste('There are ', length(setdiff(customBg, outDB$geneID)), ' genes in the background that are not in the selected annotation. Consider using a more compatible annotation with your analysis. See vignette for details on sourcing annotations and RefSeq release versions.', sep=''))
  }
