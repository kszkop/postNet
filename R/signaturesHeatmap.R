signaturesHeatmap <- function(ptn,
  ...
  if (!check_shiftUnit(unit)) {
    stop(
      "The input for 'unit' can only be 'FDR' or a percentile such as 'p25', 'p50', 'p75' (i.e., 'p' followed by an integer). \
  ...
  for (sign in seq_along(signatureList)) {
    ...
    if (unit == "FDR") {
      ...
      pval <- as.numeric(
        wilcox.test(
          as.numeric(regData[regData$signature == names(signatureList)[sign], ]$effIn),
