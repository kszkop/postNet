uorf_analysis <- function(a2sU,
  ...
  if (!is.null(comparisons)) {
    if (!checkComparisons(comparisons)) {
      stop(
        "'comparisons' must be a list of numeric vector for paired comparisons example: list(c(0,2),c(0,1)). 0 is always a background."
      )
    }
    #
    if (length(which(unique(unlist(comparisons)) == 0)) > 0 &&
        is.null(a2sU_bg(a2sU))) {
      stop(" 0 is always a background, but no background provided")
    }
  }
  if (!isStartCodon(startCodon)) {
    stop(
      "'startCodon' must be a character vector of length one, and contain only 3 nucleotide sequence, ex. 'ATG'"
    )
  }
  if (!isKozakContext(KozakContext)) {
    stop(
      "'KozakContext' must be one from these: 'strong','adequate1','adequate2','weak','any'"
    )
  }
  ...
  if (KozakContext == "strong") {
    context <-
      paste("[AG][ATGC][ATGC]", toupper(startCodon), "G", sep = "")
  } else if (KozakContext == "adequate1") {
    context <-
      paste("[AG][ATGC][ATGC]", toupper(startCodon), "[ATC]", sep = "")
  } else if (KozakContext == "adequate2") {
    context <-
      paste("[TC][ATGC][ATGC]", toupper(startCodon), "G", sep = "")
  } else if (KozakContext == "weak") {
    context <-
      paste("[TC][ATGC][ATGC]", toupper(startCodon), "[ATC]", sep = "")
  } else if (KozakContext == "any") {
    context <-
      paste("[ATGC][ATGC][ATGC]", toupper(startCodon), "[ATGC]", sep = "")
  } else {
  ...
  if (tolower(unitOut) == "number" & isTRUE(plotOut)) {
    ...
    for (i in 1:length(resOut)) {
      resProp[i] <-
        length(resOut[[i]][resOut[[i]] > 0]) / length(resOut[[i]])
    }
    # Plot
    pdf(
      ifelse(
        is.null(pdfName),
        paste("uORFs_", KozakContext, ".pdf", sep = ""),
        paste(pdfName, "_uORFs_", KozakContext, ".pdf", sep = "")
      ),
      width = 8,
      height = 8,
      useDingbats = F
    ...
    barplot(
      ...
      font = 2,
      ylim = c(0, ifelse(
        is.null(comparisons), 1, 1 + (length(comparisons) * 0.1)
      )),
      space = 0
    )
    barplot(
      resProp_strong,
      add = T,
      col = colorspace::darken(colOut, 0.25),
      xaxt = "n",
      xlab = "",
      ylab = "",
      main = "",
    ...
    if (!is.null(comparisons)) {
      for (j in 1:length(comparisons)) {
        ...
        pvalTmp <-
          format(as.numeric(
            wilcox.test(resOut[[compTmp[1]]], resOut[[compTmp[2]]], alternative = "two.sided")[3]
          ),
          scientific = TRUE,
          digits = 2)
        #
  ...
  uORFFinal[[paste('uORFs', startCodon, KozakContext, sep = '_')]] <-
    uorfOut
  #
  return(uORFFinal)
}