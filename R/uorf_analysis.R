uorf_analysis <- function(ads=NULL,
                          regulation=NULL,
                          contrast=NULL,
                          comparisons=NULL,
                          annot,
  ...
  #
  annotTmp <- regSel(annot=annotBg, region='UTR5', ext=ext)
  #Per gene
  annotBgSel <- isoSel(annot=annotTmp, method=selection)

  #
  if(KozakContext=='strong'){
    uorfOut <- as.numeric(mapply(calc_uORF, annotBgSel$seqTmp, ext= annotBgSel$extSeq, context=paste('[AG][ATGC][ATGC]',startCodon,'G',sep='')))
  } else if(KozakContext=='adequate1'){
    uorfOut <- as.numeric(mapply(calc_uORF, annotBgSel$seqTmp, ext= annotBgSel$extSeq, context=paste('[AG][ATGC][ATGC]',startCodon,'[ATC]',sep='')))
  } else if(KozakContext=='adequate2'){
    uorfOut <- as.numeric(mapply(calc_uORF, annotBgSel$seqTmp, ext= annotBgSel$extSeq, context=paste('[TC][ATGC][ATGC]',startCodon,'G',sep='')))
  } else if(KozakContext=='weak'){
    uorfOut <- as.numeric(mapply(calc_uORF, annotBgSel$seqTmp, ext= annotBgSel$extSeq, context=paste('[TC][ATGC][ATGC]',startCodon,'[ATC]',sep='')))
  } else if(KozakContext=='any'){
    uorfOut <- as.numeric(mapply(calc_uORF, annotBgSel$seqTmp, ext= annotBgSel$extSeq, context=paste('[ATGC][ATGC][ATGC]',startCodon,'[ATGC]',sep='')))
  } else {
