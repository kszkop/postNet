rfPred <- function(ptn,
  ...
  if (is.null(ptn@analysis@featureIntegration$rf)) {
    stop(
      "Please run the featureIntegration() function with the Random Forest implementation first."
    )
  }
  if (!check_number(comparison)) {
    stop(
      "Please provide the correct comparison number. You can check which are available using: ptn_check_comparisons(ptn, analysis_type='rf') "
    )
