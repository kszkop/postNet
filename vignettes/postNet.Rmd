...
```{r gettingStartedPlotFeaturesMapPNG, echo=FALSE, fig.wide = TRUE, fig.cap="UMAP visualizing changes in translation efficiency (Effect) clustered according to features identified in the omnibus model (left), with mRNAs containing SCSCGS motifs in the 5´UTR coloured (right)."}
...
The most flexible implementation of postNet allows gene lists of interest and regulatory effect measurements to be supplied from custom sources. A `postNetData` object is initialized using the `geneList`, `effectMeasure`, and `customBg` parameters. The gene sets provided with `geneList` can be compared in downstream statistical analyses. A custom background can also be provided to `customBg` that will be used for statistical comparisons against the gene sets of interest. Note that the background should correspond to the set of genes or transcripts that was quantified (i.e., all genes used as input in the analysis that generated the regulatory effect measurement `effectMeasure`). For example, all genes in a given data set passing expression/reproducibility thresholds. If no custom background is provided, the background will be set as all genes IDs present in `geneList`. Although optional, it is strongly recommended to carefully consider that the appropriate background is used.
...
If using \code{ads}, the appropriate background is automatically retrieved from the \code{Anota2seqDataSet} object, and \code{customBg} should be \code{NULL}. 

```{r GetAds, eval=TRUE, echo=TRUE}
...
This postNet analysis will allow identification of mRNA features and comparison between translationally activated and suppressed genes specified by the regulatory modes selected with the `regulation` parameter. Feature integration analysis will model changes in translation efficiency, specified by `regulationGen`.

```{r postNetStartAds, eval=TRUE, echo=TRUE}
...
```{r slopeFiltrm, echo = FALSE, message = FALSE, results = 'hide', eval = TRUE}
