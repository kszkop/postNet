...
These inputs are compiled at the start of the analysis by initializing a `postNetData` object. This class of object serves as the container for storing inputs and some outputs generated during the analysis, and is designated by `ptn` in the example code provided here. The following sections describe the different options and customization available, along with examples illustrating how to set up a postNet analysis.

## Reference sequence annotations

Reference sequence annotations corresponding to the 5'UTR, CDS, and 3'UTR regions of mRNAs, including in-built, custom, or retrieved directly from the NCBI RefSeq database [@OLeary2016]. Optionally, UTR sequences can also be adjusted if more precise sequences are available (e.g. from CAGE, QuantSeq, long-read sequencing, etc.)

### Loading in-built sequence annotations

### Retrieving sequence annotations from the NCBI RefSeq database

### Using custom sequence annotations
