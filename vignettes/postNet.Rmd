...
Nucleotide content, particularly GC content, is also strongly associated with the post-transcriptional fate of mRNAs [ref]. For each gene, the `contentAnalysis()` function computes and returns a list of percentage of nucleotide content of each mRNA sequence region specified by the `region` parameter. It is also possible to select more specific [sub-regions](#subregions) to either examine or exclude from the analysis. [Statistical comparisons](#comparisons) and [visualizations](#plottingOpts) can be generated using the parameters described above.

 \item{contentIn}{A character vector specifying one or more nucleotide(s) or nucleotide combinations to quantify. These can be any selection or combination of A, T, G, or C. For example \code{c("GC")}, or \code{c("G", "A")}. When \code{region} includes \code{"CDS"}, nucleotide content at specific codon positions can also be quantified. For example, \code{c("GC3")} calculates GC content in the third codon position, and \code{c("A12")} calculates the percentage of A content in the first and second codon positions. 
