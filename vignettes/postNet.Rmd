...
# Compile a list of features:
 myFeatureList <- c(len, GC_content[c(1,3)], GC3_content, FE, uORFs, UTR5_SCSCGS_num, codonCounts, signatureFeatures)
...
Stepwise regression modelling and network analysis are specified in the `featureIntegration()` function by setting the `analysis_type` parameter to `lm`. The regulatory effect measurement is then modelled using the input features in three phases to generate univariate, omnibus, and adjusted models (described below). In the first phase, univariate models are used to assess the association of each feature to the regulatory effect measurement.   
