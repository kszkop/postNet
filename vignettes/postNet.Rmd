...
Generally Applicable Gene-set Enrichement (GAGE) analysis can also be performed using the regulatory effect measurement contained in a `postNetData` object. The `gageAnalysis()` function implements the [gage](gage) package [ref], and can be used with the "BP", "MF", or "CC" Gene Ontology categories, and well as with 'KEGG' pathways. These are specified using the `category` parameter.  
...
The example code below performs GAGE analysis using the "CC" GO term category, and extracts terms that are significantly associated with increased log2 fold changes in translation efficiency. 
