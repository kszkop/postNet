...
The `featureIntegration()` function is used to model changes in post-transcriptional regulation and identify cis and/or trans factors (features of mRNA molecules, signatures of upstream regulators, etc.) that can explain the observed regulatory effects. The method integrates mRNA features and signatures obtained in previous steps of the postNet workflow (or from custom sources), and has several options for implementation, including stepwise linear regression modelling and network analysis, and Random Forest feature selection and classification. These topics and considerations for use will be discussed in the following sections. 
...
# Compile a list of features:
 myFeatureList <- c(len[c(1)], GC_content[c(1)], GC3_content, FE[c(1)], uORFs, UTR5_SCSCGS_num, codonCounts, signatureFeatures)
...
## Post-transcriptional network modelling with forward stepwise linear regression
...
### Running stepwise linear regression analysis

Stepwise regression modelling and network analysis are specified in the `featureIntegration()` function by setting the `analysis_type` parameter to `lm`. A number of parameters can be used to control the behaviour of the modelling. Similarly to the functions previously described, [statistical comparisons](#comparisons) can be specified using the `comparisons` parameter. It is possible to perform multiple pair-wise comparisons with stepwise regression modelling. By default, regression modelling is performed using only the set of regulated genes (i.e., those included in `geneList`, or `regulation` if starting from an `Anota2seqDataSet` object). However, it is also possible to perform the modelling with all genes included in the data set by setting the `regOnly` parameter to `FALSE`. 
...
In the example code below the three phases of the forward stepwise regression modelling will be performed using the list of input features defined above. In this analysis, only the sets of regulated genes will be included in the analysis, comparing translationally activated and suppressed genes. Those features passing the *FDR* threshold of 0.05 in univariate models will be included in stepwise regression.   
...
The results of the modelling will be stored in the `postNetData` object, and a series of PDFs will be produced. The PDF with the suffix "FinalModel" will contain the summary tables shown above in Figures 13-15. Using the code below, it is possible the retrieve the full results from each phase of modelling, as well as the set of features that were selected in the omnibus model that significantly explain changes in the regulatory effect. Furthermore, if multiple modelling using multiple pairwise was performed, you can check which modelling results are available.  
...
# Retrieve the stepwise regression results
...
## Retreiving models and results from the postNetData object 
...
## microRNA enrichment analysis with a postNetData object
...
```{r plotSignatures, eval=FALSE, echo=TRUE}
...
# Examine the regulation of mTOR-sensitive transcripts 
 plotSignatures(ptn = ptn,
                               ...
                               xlim = c(-3,3), 
                               tableCex = 1,
                               pdfName = "Example_mTORsignatures")
```

```{r plotSignaturesPDF, eval=FALSE, echo=FALSE}
# Convert to PNG
image_read_pdf("mTORsignatures.pdf", density = 300) %>%
  image_resize("3000x") %>%
  image_write("Figures/mTORsignatures.pdf")

```

```{r plotSignaturesPNG, eval=FALSE, echo=FALSE, fig.wide = TRUE, fig.cap=""}
knitr::include_graphics("Figures/mTORsignatures.pdf")

```

```{r plotSignaturesHide, eval=FALSE, echo = FALSE, message = FALSE, results = 'hide', eval = TRUE}
unlink(c("mTORsignatures.pdf"))
```
...
The example code below will load the set of signatures included with the package, and assess the regulation of the signatures for genes whose translation is either activated or suppressed by activation of the integrated stress response with thapsigargin.  
```{r plotSignatures_ads, eval=TRUE, echo=TRUE, results='hide'}
...
```{r plotSignatures_adsPDF, eval=TRUE, echo=FALSE}
# Convert to PNG
...
```{r plotSignatures_adsPNG, echo=FALSE, fig.wide = TRUE, fig.cap="Scatterplot of total vs. translated mRNA from an example anota2seq analysis with the location of genes belonging to signatures of interest colored (left panel). Subsequent panels show the eCDFs of log2 fold changes in translation and buffering regulatory modes, as well as for translated and total mRNA for the signatures of interest. Grey curves indicate the background gene set. Wilcoxon p-values and differences between the distributions at each quantile are indicated. A shift to the right indicates and increase compared to background, while shift to the left indicates a decrease."}
knitr::include_graphics("Figures/Example_ISRsignatures_data_Osmosis_1h_signature_ISR.png")

```

```{r plotSignatures_adsHide, echo = FALSE, message = FALSE, results = 'hide', eval = TRUE}
