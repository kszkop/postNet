...
```{r GOplottingPNG, echo=FALSE, fig.wide = TRUE, fig.cap="A dot plot visualizing GO term enrichment analysis results."}
...
Several gene signatures relevant to post-transcriptional regulation are included with the package.
...
```{r plotSignaturesPNG, eval=FALSE, echo=FALSE, fig.wide = TRUE, fig.cap="A plot visualizing GSEA results for the enrichment of the gene Set1 in the example dataset."}
...
When starting from from the results of an `anota2seq` analysis, gene signatures can be assessed using the `plotSignatures_ads` function. The key difference from the custom gene list workflow is that instead of providing the `postNetData` object, the `anota2seqDataSet` object (the `ads` parameter) must be provided instead. Four eCDFs will be generated examining the distribution of log2 fold changes in total mRNA, polysome-associated mRNA, translation, and buffering (also known as offsetting) for each gene signature provided. In addition to eCDFs, a scatter plot of total mRNA vs. polysome-associated mRNA will also be generated, with the genes corresponding to the signatures provided coloured. 
...
# Examine the regulation of mRNAs with TOP motifs in the results of an anota2seq analysis :
...
```{r plotSignatures_adsPNG, echo=FALSE, fig.wide = TRUE, fig.cap="A plot visualizing GSEA results for the enrichment of the gene Set1 in the example dataset."}
...
Finally, it is also possible to generate a heatmap comparing the regulation of multiple gene signatures in your data set of interest using the `signaturesHeatmap` function. 

a The \code{signaturesHeatmap} function produces a heatmap of the regulation of gene signatures of interest in a 
