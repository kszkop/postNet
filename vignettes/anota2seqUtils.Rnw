...
---
title: | 
  <center> Anota2seqUtils  </center>
  <center> integration of mRNA regulatory features <center>
output: rmarkdown::html_vignette
description: >
  mRNAs often harbor multiple features that can impact their regulation. To understand underlying mechanism of the regulation, it is   important     not only to identify cis- and trans-acting factors but also to understand relationship between them. Anota2seqUtils        includes methods to     identify, quantify and perform statistical analysis to assess how the compendium of these features impacts on    mRNA fate, while allowing         establishment of the functional relationship between them. 
---




%\VignetteIndexEntry{Anota2seqUtils for integration of mRNA regulatory features}
%\VignetteKeywords{Translation, feature integration}
...
\bioctitl{Anota2seqUtils for integration of mRNA regulatory features}
\author{Krzysztof J. Szkop, Ola Larsson (+ add later) \thanks{\email{krzysztof.szkop@ki.se, ola.larsson@se}}}
\maketitle
\tableofcontents
...
The package was primarily designed to be complementary to anota2seq \cite{Oertlin} method for analysis of differential translation, so it is sufficient to have an output of function \Rfunction{anota2seq::anota2seqRun} to perform all the steps. However, it is also possible to expand analysis by providing additional genes or even use own gene selection and own values that might not necessarily represent translational efficiences from anota2seq analysis.\\



---
title: "Anota2seqUtils for integration of mRNA regulatory features"
output: rmarkdown::html_vignette
description: >
mRNAs often harbor multiple features that can impact their regulation. To understand underlying mechanism of the regulation, it is important not only to identify cis- and trans-acting factors but also to understand relationship between them. Anota2seqUtils includes methods to identify, quantify and perform statistical analysis to assess how the compendium of these features impacts on mRNA fate, while allowing establishment of the functional relationship between them. 

vignette: >
  %\VignetteIndexEntry{Introduction to anota2seqUtils}
...
\bioctitle[Anota2seqUtils for integration of mRNA regulatory features]{Anota2seqUtils\\
Anota2seqUtils for integration of mRNA regulatory features}
\author{Krzysztof Szkop, Ola Larsson + add later \thanks{\email{krzysztof.szkop@ki.se , ola.larsson@ki.se _ add later}}}
\maketitle
\tableofcontents
...
\section{Quick start}
The easiest way to start is by using anota2seqUtils wrapper function \Rfunction{anota2seqUtilsRun}. 
<<Quick start, echo = TRUE, message = FALSE, results = 'hide', eval = TRUE >>=
library(anota2seqUtils)
data(anota2seqUtils_data)

@

<<gettingStarted, echo = TRUE, message = FALSE, results = 'hide', eval = TRUE >>=
utilsOut <- anota2seqUtilsRun(species='human',regionSel=c('UTR5','CDS','UTR3'), ads = anota2seqUtils_exampleData, regulation = c("translationUp","translationDown"),contrast = c(1,1), comparisons = list(c(1,2)),contentIn=list('G','C','A','T'),addSign=addSign, regulationGen='translation', contrastSel=1, miRNATargetScanFile='Predicted_Targets_Context_Scores.txt',analysis_type='lm',memePath='/opt/local/bin/')
@
...
\item Analysis of folding energies \Rfunction{foldEnergyAnalysis}.
...
Here we show an overview of the whole step by step workflow. We illustrate an analysis using example anota2seq object available with the package.
...
The reference sequences for each mRNA region are the base of all step analysis. It is possible to use existing pre-prepared ones. These are obtained from RefSeq database. At the moment these are only available for human and mouse. To check available versions please run function \R{checkAvailableVersions}:

<<load_library_and_data, echo=TRUE, eval = TRUE >>=
checkAvailableVersions(species='human')
@

However, it is possible to create new annotation files based on latest ncbi refseq release (option= 'create'). It is automatic process for human and mouse. For other species option = 'createFromFile' is required and files rna.fna.gz, rna.gbff.gz and genomic.gff.gz needs to be manually downloaded from ncbi refseq and provided. It is also possible to simply load (option='custom') your own version in appropriate format, i.e. data.frame for transcripts IDs, gene IDs, and sequences for 5'UTR, CDS and 3'UTR with respective column names: id, geneID, UTR5_seq, CDS_seq, UTR3_seq.
...
G4_utr5 <- contentMotifs(ads = ads, annot = annot, regulation =  c("translationUp","translationDown"), contrast = c(1,1), motif='G4',region = 'UTR5', len=1, selection ='random', comparisons = list(c(1,2)))

@

<<foldEnergyAnalysis, echo = TRUE, message = FALSE, results = 'hide', eval = TRUE >>=
fe_utr5 <- foldEnergyAnalysis(ads=ads, annot=annot, regulation =  c("translationUp","translationDown"), contrast =  c(1,1), region='UTR5',selection='random', comparisons = list(c(0,1),c(0,2),c(1,2)), source='load', version='rel_109.20201120',species = 'human', resid = FALSE, onlyRun=FALSE, plotType='ecdf', pdfName= NULL)
@


<<codonUsage, echo = TRUE, message = FALSE, results = 'hide', eval = TRUE >>=
codonOut <- codonUsage(ads = ads, analysis='codon', regulation =  c("translationUp","translationDown"),contrast = c(1,1),type='sequence', comparisons = list(c(1,2)),annotType='ccds',source='load',selection='random', pAdj=0.01, plotHeatmap=TRUE, pdfName= NULL, species='human')

selCodonOut1 <- codonCalc(codonIn=codonOut_c1_trans,analysis='codon', featsel=c('CTG','GAG','AAG','CAG'),unit='count',regulation =  c("totalmRNADown","translatedmRNAUp"),contrast = c(1,1), comparisons = list(c(1,2)),ads=adsVoom,pdfName = 'Up')
@



TOPmotif_sign <- signCalc(ads = adsVoom, signature=as.character(TOPmotifgenes))
