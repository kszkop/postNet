...
\section{Quick start}
The easiest way to start is using anota2seqUtils wrapper function \Rfunction{anota2seqUtilsRun}. 
<<Quick start, echo = TRUE, message = FALSE, results = 'hide', eval = TRUE >>=
library(anota2seqUtils)
data(anota2seqUtils_exampleData)
@
