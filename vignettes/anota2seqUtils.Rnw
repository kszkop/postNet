...
Post-transcriptional regulation of gene expression occurs at multiple levels including perturbations in mRNA stability and translation (REF). The selectivity of post-transcriptional regulatory mechanisms is chiefly achieved via the interplay between a plethora of cis- (i.e., mRNA elements and features) and trans-acting factors (e.g., RBPs, miRNAs) (REF). This allows transcript- and context-dependent alterations in mRNA abundance and/or translation efficiency thereby resulting in selective remodeling of the proteome that allows adaptation to various environmental challenges  [reviewed in (Jonas and Izaurralde, 2015; Hinnebusch, Ivanov and Sonenberg, 2016; Roux and Topisirovic, 2018)]. For example, 5’ untranslated region (5’UTR) GC content is negatively correlated with translational efficiency (Pelletier and Sonenberg, 1985; Manzella and Blackshear, 1990). The inhibitory effect of high 5’UTR GC content is thought to be a consequence of formation of secondary structures that impede scanning of the ribosome (Waldron, Raza and Le Quesne, 2018). Translation of mRNAs with high GC content in their 5’UTR critically depend on the activity of RNA helicases including eukaryotic translation initiation factor 4A (eIF4A) (Svitkin et al., 2001; Waldron, Raza and Le Quesne, 2018). eIF4A gains processivity within the eIF4F complex, which also includes 5’ cap-binding protein eIF4E and scaffold protein eIF4G (Sonenberg and Hinnebusch, 2009). eIF4F complex formation is stimulated by the mechanistic target of rapamycin (mTOR)  (Roux and Topisirovic, 2018). mTOR therefore controls translation of mRNAs with structured 5’UTRs (REF). Moreover, mTOR stimulates translation of mRNAs harboring short 5’UTRs (<30 nt) that are highly sensitive to alterations in eIF4E availability, but only marginally to changes in eIF4A levels and/or activity (REF). Finally, mTOR stimulates translation of mRNAs harboring 5’-terminal oligopyrimidine (TOP) motif via phosphorylation of La-related protein 1(LARP1) (REF). Notably, mRNAs that carry features that render their translation mTOR-sensitive appear to be highly functionally related (REF). Specifically, mRNAs with high GC content, short 5’UTRs and TOP motifs are highly enriched in those encoding factors involved in proliferation and survival, proteins with mitochondrial functions and components of the translation apparatus, respectively (REF). In response to a variety of stimuli mTOR can therefore engage different effectors to selectively modulate the composition of the proteome and drive proliferation, mitochondrial metabolism and energy production, and protein synthesis (REF). In addition to the properties of 5’UTR, translation is also affected by the composition of coding region and 3’UTR features (REF). Individual mRNAs, however, frequently harbor not just one, but many of the features that can impact on their regulation. This makes identification of RNA features underlying post-transcriptional programs challenging, and may lead to erroneous conclusions when RNA features covary across transcripts. Yet, to date, there are no in silico approaches to systematically identify, integrate and model how well RNA features associate with e.g. translational efficiencies obtained from transcriptome-wide studies of mRNA translation employing polysome- (Johannes et al., 1999) or ribosome-profiling (Ingolia et al., 2009). Therefore, we developed anota2seqUtils, a bioconductor package designed to systematically identify and integrate regulatory mRNA features. Anota2seqUtils includes methods to identify and quantify RNA features (e.g., GC content, length and folding energy of UTRs and CDS, RNA motifs, uORFs, codon composition etc.) associated with selective alterations in different steps of post-transcriptional regulation of gene expression (e.g., mRNA translation and stability). This is coupled with statistical analysis to assess how the compendium of identified features impacts on mRNA fate, while allowing establishment of the functional relationship between different features of mRNA. To evaluate the method, we investigated the effects of perturbations in mRNA abundance and translation efficiency under the conditions where cells were acutely exposed to insulin or different stressors (e.g., mTOR inhibitors, serum depletion, hypoxia).  Anota2seqUtils efficiently captured previously established features that modulate mRNA translation under these conditions, which verified its optimal performance. In addition, anota2seqUtils captured hitherto unappreciated 3’UTR features that render mRNAs susceptible to translational offsetting. Transcripts harboring these 3’UTR features exhibit longer half-lives than mRNAs that are not subjected to translational offsetting. This suggest that translational offsetting may be coupled with mRNA stability. In summary, we present anota2seqUtils for identification and integration of regulatory mRNA features on a transcriptome-wide scale and unravel previously undescribed transcript characteristics that underpin translational offsetting.
...
\item Selection of features and classification into modes for regulation of gene expression using \Rfunction{anota2seqSelSigGenes} and \Rfunction{anota2seqRegModes}.  See section \ref{genesSection} and \ref{regModesSection}, respectively.
...
anota2seqUtils provides a wrapper function called \Rfunction{anota2seqUtilsRun} which performs all analysis steps with relevant default parameters. Here we show an overview of the whole workflow using this function. We illustrate an analysis using count data from an RNA sequencing experiment on total mRNA (called anota2seq\_data\_T here) and on translated mRNA (polysome-associated mRNA or Ribosome Protected Fragments, data called anota2seq\_data\_P) on conditions ("ctrl" and "treatment") and a vector of sample annotation (called anota2seq\_pheno\_vec). The following code performs normalization, assesses model assumptions and performs the analysis for the default contrast (treatment vs. control in this case):
